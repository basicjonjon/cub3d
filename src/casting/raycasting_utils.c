...
/*   Updated: 2025/07/29 12:14:00 by mmarpaul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_hit(t_data *data, t_ray *ray, char **map, int *side)
{
	while (1)
	{
		if (ray->mapX < 0 || ray->mapX >= data->param.mapX
			|| ray->mapY < 0 || ray->mapY >= data->param.mapY)
			break ;
		...
		if (ray->mapY >= 0 && ray->mapY < data->param.mapY
			&& ray->mapX >= 0 && ray->mapX < data->param.mapX
