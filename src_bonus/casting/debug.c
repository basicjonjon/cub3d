...
/*   Updated: 2025/07/22 14:36:27 by jle-doua         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	calc_rays(t_data *data, char **map, float ray_angle, int *hit_x,
		int *hit_y)
{
	float	x;
	float	y;
	...
	while (!hit)
	{
		if (mapX < 0 || mapX >= data->param.mapX || mapY < 0
			|| mapY >= data->param.mapY)
			break ;
		...
		if (mapY >= 0 && mapY < data->param.mapY && mapX >= 0
			&& mapX < data->param.mapX && map[mapY][mapX] == '1')
			hit = 1;
	}
	dist = 0;
	if (hit_x && hit_y)
...
// 		// cast_rays(data, player->x * BLOCK, player->y * BLOCK, hit_x, hit_y,
			// HGREEN);
// 		i++;
// 	}
// }
...
void	draw_rays(t_data *data, t_player *player, t_config *c)
{
	...
	column_width = screenWidth / NBR_RAYS + 1;
...
int	debug(t_data *data)
{
	...
	{
		// draw_player(data->player.posX, data->player.posY, playerSize, HBLACK,
			// data);
		clear_image(&data->img, screenWidth, screenHeight);
		move_player(data, &data->player, &data->conf);
		// draw_map(data);
		// draw_player(data->player.x * BLOCK, data->player.y * BLOCK,
			// playerSize, HRED, data);
